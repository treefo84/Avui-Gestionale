...
import { X, UserPlus, Trash2, Shield, LifeBuoy, Pencil, Save, ChevronLeft, RefreshCw, KeyRound, Mail, Briefcase, UserCog, Phone, Cake } from 'lucide-react';
...
interface UserManagementModalProps {
  ...
  onAddUser: (name: string, role: Role, email: string, isAdmin: boolean, phoneNumber: string, birthDate: string) => void;
...
export const UserManagementModal: React.FC<UserManagementModalProps> = ({
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (newName.trim()) {
      onAddUser(newName, newRole, newEmail, newIsAdmin, newPhone, newBirthDate);
      setNewName('');
      setNewEmail('');
      setNewPhone('');
      setNewBirthDate('');
  ...
  const handleAvatarUpdate = (seed: string) => {
      ...
      // Update local state to reflect change immediately if we are still editing
      setEditingUser({ ...editingUser, avatar: newAvatarUrl });
  };

  // Sort users: Instructors/Managers first, then Helpers, then Reserves
  const sortedUsers = [...users].sort((a, b) => {
      const roleOrder = { [Role.MANAGER]: 0, [Role.INSTRUCTOR]: 1, [Role.HELPER]: 2, [Role.RESERVE]: 3 };
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-3">
             {editingUser && (
                 <button onClick={() => setEditingUser(null)} className="p-1 hover:bg-slate-200 rounded-full transition-colors">
                     <ChevronLeft size={24} />
                 </button>
             )}
             <div>
          ...
          {editingUser ? (
            ...
            <div className="space-y-6 animate-in slide-in-from-right duration-200">
               ...
               <div className="flex flex-col items-center justify-center mb-6">
                   <img src={editingUser.avatar} className="w-24 h-24 rounded-full border-4 border-slate-100 mb-4 bg-slate-50" />
                   <div className="flex gap-2 max-w-full overflow-x-auto p-2">
                       {AVATAR_SEEDS.slice(0, 8).map(seed => (
                           <button key={seed} onClick={() => handleAvatarUpdate(seed)} className="w-8 h-8 rounded-full border border-slate-200 hover:scale-110 transition-transform overflow-hidden">
                               <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${seed}&backgroundColor=${['b6e3f4','c0aede','ffdfbf'][Math.floor(Math.random()*3)]}`} />
                           </button>
                       ))}
                       <button onClick={() => handleAvatarUpdate(Math.random().toString())} className="w-8 h-8 rounded-full border border-slate-200 bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-blue-50 hover:text-blue-500">
                           <RefreshCw size={14} />
                       </button>
                   </div>
               ...
               <form onSubmit={handleEditSave} className="space-y-4">
                  ...
                  <div className="pt-4 flex gap-3">
                      <button type="button" onClick={() => setEditingUser(null)} className="flex-1 py-3 text-slate-600 font-bold hover:bg-slate-100 rounded-xl transition-colors">
                          Annulla
                      </button>
                      <button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">
            ...
            <>
                ...
                <section className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                    ...
                    <div className="md:col-span-12 mt-2">
                        <button type="submit" className="w-full bg-blue-600 text-white text-sm font-bold py-2 rounded-lg hover:bg-blue-700 transition-colors">
                        Arruola (Password default: 1234)
                        </button>
                    </div>
                    </form>
                </section>
                ...
                {/* User List */}
                <section>
                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Libro Mastro dell'Equipaggio</h3>
                    <div className="space-y-2">
                    {sortedUsers.map(user => {
                        let roleBadge = "bg-emerald-100 text-emerald-700";
                        let RoleIcon = LifeBuoy;
                        let roleName = "Mozzo";
                        ...
                        return (
                        <div key={user.id} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl hover:shadow-sm transition-shadow group">
                        ...
                        >
                            <img src={user.avatar} alt={user.name} className={`w-10 h-10 rounded-full border border-slate-200 ${user.role === Role.RESERVE ? 'grayscale opacity-70' : ''}`} />
                            <div>
                                <h4 className="font-bold text-slate-800 flex items-center gap-2">
                                    {user.name} 
                                    {user.isAdmin && <span className="bg-slate-800 text-white text-[10px] px-1.5 rounded border border-slate-700">ADMIN</span>}
                                    {user.id === currentUserId && <span className="text-xs text-blue-500">(Tu)</span>}
                                </h4>
                                <div className="flex flex-col">
                                    <p className="text-xs text-slate-500 truncate max-w-[150px]">{user.username}</p>
                                    <div className="flex gap-2">
                                         {user.email && <p className="text-[10px] text-slate-400 truncate flex items-center gap-0.5"><Mail size={8}/> {user.email}</p>}
                                         {user.phoneNumber && <p className="text-[10px] text-slate-400 truncate flex items-center gap-0.5"><Phone size={8}/> {user.phoneNumber}</p>}
                                    </div>
                                </div>
                            </div>
                        </div>
