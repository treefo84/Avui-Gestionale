...
import { Activity, Assignment, AssignmentStatus, Boat, User, DayNote } from '../types';
import { Ship, Anchor, LifeBuoy, Skull, Ban, MessageCircle } from 'lucide-react';
...
export const DayHoverModal: React.FC<DayHoverModalProps> = ({ date, position, data, notes }) => {
      ...
      <div className="p-3 space-y-3 bg-slate-50">
        ...
        {notes.length > 0 && (
            <div className="bg-amber-50 p-2 rounded-lg border border-amber-200 mb-2">
                ...
                {notes.map(note => (
                    <p key={note.id} className="text-xs text-slate-700 leading-tight italic border-l-2 border-amber-300 pl-2 mb-1 last:mb-0">
                        "{note.text}"
                    </p>
                ))}
            </div>
        ...
        {data.map((item, idx) => {
            ...
            return (
              <div key={idx} className={`bg-white p-2 rounded-lg border shadow-sm relative overflow-hidden ${isCancelled ? 'border-red-200' : 'border-slate-200'}`}>
                 {/* Decorative accent */}
                 <div className={`absolute left-0 top-0 bottom-0 w-1 ${isCancelled ? 'bg-red-500' : 'bg-blue-500'}`}></div>
                 
                 <div className="flex items-start justify-between mb-2 pl-2">
                    <div>
                        <span className="text-[10px] font-bold uppercase text-slate-400 tracking-wider block">{item.boat.name}</span>
                        <h5 className={`font-bold text-sm leading-tight ${isCancelled ? 'text-red-600 line-through' : 'text-slate-800'}`}>
                            {item.activity.name}
                        </h5>
                        {isCancelled && <span className="text-[10px] font-bold text-red-500 uppercase tracking-widest">ANNULLATA</span>}
                    </div>
                 </div>
                 ...
                 <div className={`space-y-1.5 pl-2 ${isCancelled ? 'opacity-50 grayscale' : ''}`}>
                    ...
                    <div className="flex items-center gap-2">
                       {item.instructor ? (
                           <>
                             <img src={item.instructor.avatar} className="w-5 h-5 rounded-full border border-slate-200" alt="I" />
                             <span className="text-xs font-medium text-slate-700">{item.instructor.name}</span>
                           </>
                       ) : (
                           <>
                    ...
                    <div className="flex items-center gap-2">
                       {item.helper ? (
                           <>
                             <img src={item.helper.avatar} className="w-5 h-5 rounded-full border border-slate-200" alt="H" />
                             <span className="text-xs font-medium text-slate-700">{item.helper.name}</span>
                           </>
                       ) : (
                           <>
                             <div className="w-5 h-5 rounded-full bg-slate-100 flex items-center justify-center border border-slate-200">
